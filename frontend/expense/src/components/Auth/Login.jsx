    ...
    const handleLogin = async () => {
        try {
            ...
            if (response.ok) {
                ...
                localStorage.setItem('isPremium', result.isPremium.toString());  
                navigate('/home');
            } else {
        ...
        <div>
            ...
            <div className='lg:w-3/12 md:w-5/6 mx-3 md:mx-auto right-0 left-0 bg-cyan-900 rounded-lg my-24 md:p-8 lg:p-11 p-4 text-center'>
                ...
                <form onSubmit={(e) => e.preventDefault()}>
                    <input className='p-2 outline-none bg-slate-300 w-full m-2' ref={email} type='email' placeholder='email'/>
                    <input className='p-2 outline-none bg-slate-300 w-full m-2' ref={password} type='password' placeholder='password'/>
                    <p className='p-2 text-red-600'>{errorMessage}</p>  
                    <button className='p-2 m-2 bg-teal-500 hover:bg-teal-700 w-full text-white rounded-md' onClick={handleLogin}>
                        Login
                    </button>
